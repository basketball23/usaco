...
int main() {
    ...
    long long single_enc_area = 1LL * (xmax - xmin) * (ymax - ymin);
    long long double_enc_area = 9223372036854775807;
    ...
    */
    ...
    for (int i = 0; i < N - 1; i++) {
        ...
        long long left_area = 1LL * (left_r - left_l) * (limit_list_left[i]);
        long long right_area = 1LL * (right_r - right_l) * (limit_list_right[i + 1]);
    ...
    // Horizontal loop sorting and running by y values
    ...
    for (int i = 0; i < N - 1; i++) {
        ...
        long long bottom_area = 1LL * (bottom_t - bottom_b) * (limit_list_bottom[i]);
        long long top_area = 1LL * (top_t - top_b) * (limit_list_top[i + 1]);
